...
for comp = 1:r
    ...
    for exp = 1:c
        if ~isempty(alldat(comp,exp).data)
            ...
            auc_base = trapz(abs(signal_base));
            signal_expo = filt1d_select(length(filt1d_select)/2+1:length(filt1d_select));
            auc_expo = trapz(abs(signal_expo));
            auc_norm = auc_expo/auc_base; 
            
            %% Saves variables into structure
            auc_info(comp).expo(end+1) = auc_expo;
            auc_info(comp).base(end+1) = auc_base;
            auc_info(comp).norm(end+1) = auc_norm;
...
for comp = 1:7
    ...
    for idx1 = 1:length(sel)
        auc_expo = [auc_expo,auc_info(sel(idx1)).expo];
        auc_base = [auc_base,auc_info(sel(idx1)).base];
        auc_norm = [auc_norm,auc_info(sel(idx1)).norm];
    end
  
    [p_auc(comp),h(comp),s] = signrank(auc_base,...
        auc_expo);
 stats = [stats,s];
    
    avg_auc_expo(comp) = mean(auc_expo);
    avg_auc_base(comp) = mean(auc_base);
    avg_auc_norm(comp) = mean(auc_norm)*100;    
    auc_norm_struct(comp).norm = auc_norm; 
    sem_auc_norm(comp) = std(auc_norm)/sqrt(length(auc_norm))*100;
end
...
for idx1 = 1:length(names)
    s = select(idx1,:)>0;
    for idx2 = 1:sum(s)
        dat_vec = auc_info(select(idx1,idx2)).norm;
        col = c2(idx2,:);
        for idx3 = 1:length(dat_vec)
            scatter(100*dat_vec(idx3),names(idx1),20,col,'filled',...
...
xlabel('% of Baseline')
title('AUC of Absolute Signal Value','FontWeight','Normal')
hold on
...
fig_path = 'Final Figures/Avg AUC Bar graph stats ';
